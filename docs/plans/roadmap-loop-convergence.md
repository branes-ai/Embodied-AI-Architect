...
|---|------|----------|-------|--------|
| S1 | **Schema audit** — every field any node writes must be declared in `DesignState`, or LangGraph silently drops it at runtime (found `final_report` / `research_citations` / `pending_specialist_tasks` this way). Exhaustively reconcile all node writes against the schema before deleting the old schemas. | `design_state.py` | 1 | ☑ (#204) |
| S2 | **State migration + compat shims** — subsume `SoCDesignState` + `OptimizationLoopState`; keep existing `moo_results` / snapshot / API consumers (CLAUDE.md field table) resolving; run the full test suite as the gate. S2a: `optimization_loop.py` ☑ (#205). S2b: `soc_graph.py`/`dispatcher.py` ☑ (#206). S2c: `SoCDesignState` class deleted; kept as a lazy `__getattr__` compat alias to `DesignState` so existing imports/annotations keep working ☑ (#207). | `design_state.py`, `soc_state.py`, `optimization_loop.py` | 1 | ☑ |
| S3 | **`DesignDelta.change` discriminated union** — replace the free-form `dict` payload with a per-`DeltaKind` typed model so edits validate at the boundary. | `design_state.py` | 1 | ☑ (#208) |
| S4 | **LLM critic path** — `Critic._review_with_llm` is a documented `NotImplementedError`. Assemble prompt from PPA + Pareto + `open_issues` + retrieved research (reuse `optimization_loop._reason_with_llm` shape), require JSON, parse into `CriticVerdict`. | `loop_agents.py` | 2 | ☑ (#209) |
| S5 | **Research-grounded deltas** — `_default_delta_for` presets are crude per-metric guesses (power→int8, latency→more MAC rows). The LLM path should propose targeted deltas grounded in the research library. | `loop_agents.py` | 2 | ☑ (#210) |
| S6 | **Real `_merge_pareto`** — the accumulation stub in the MOO tool must reuse `moo.specialist._merge_pareto_frontiers` (monotonic; regression test for frontier monotonicity). | `loop_convergence_graph.py` | 2 | ☑ (#211) |

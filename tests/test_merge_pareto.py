...
def _powers(points: list[dict]) -> set[float]:
    return {p["objectives"]["power_watts"] for p in points}


def test_first_merge_returns_new_points():
    merged = _merge_pareto([], [_pt(5.0, 20.0), _pt(4.0, 30.0)])
    assert _powers(merged) == {5.0, 4.0}


def test_dominated_new_point_is_dropped():
    ...
    assert 5.0 in _powers(merged)  # A survived
    assert 4.0 in _powers(merged)  # C added
    assert 6.0 not in _powers(merged)  # B dominated -> dropped


def test_non_dominated_points_never_lost_when_only_dominated_added():
    frontier = _merge_pareto([], [_pt(5.0, 20.0), _pt(4.0, 30.0)])
    before = _powers(frontier)
    ...
    assert before.issubset(_powers(frontier))


def test_new_point_that_dominates_accumulated_removes_it():
    ...
    assert _powers(merged) == {5.0}


def test_points_keep_engine_native_shape():
    merged = _merge_pareto([], [_pt(5.0, 20.0)])
    # Consumers (evaluate_node/recommend_node) read point["objectives"][...].
    assert merged[0]["objectives"]["power_watts"] == 5.0
